	...
	<div class="container">
		...
		<div class="row">
			...
			<div id="content" class="col-md-9 sticky-content">
                ...
                @endif
				<a  href="{{route('reseller.orderHistory')}}"><h2 style="margin-bottom: 10px;" class="title">Total Order ({{$all}})</h2></a>
				<div class="row">
		                ...
		                <div class="card label-info">
		                    <div class="user-box">
		                        ...
		                        <div class="d-flex   no-block align-items-center">
		                            <a href="{{route('reseller.orderHistory', 'pending')}}" class="link ml-auto">{{$pending}}</a>
		                        </div>
		                    </div>
		                </div>
		            </div>
		            <div class="col-md-2 col-xs-6">
		                <div class="card label-warning">
		                    <div class="user-box">
		                        ...
		                        <div class="d-flex   no-block align-items-center">
		                            <a href="{{route('reseller.orderHistory', 'accepted')}}" class="link ml-auto">{{$accepted}}</a>
		                        </div>
		                    </div>
		                </div>
		            </div>
		            <div class="col-md-2 col-xs-6">
		                <div class="card label-default">
		                    <div class="user-box">
		                        ...
		                        <div class="d-flex  no-block align-items-center">
		                            <a href="{{route('reseller.orderHistory', 'ready-to-ship')}}" class="link ml-auto">{{$readyToship}}</a>
		                        </div>
		                    </div>
		                </div>
		            </div>
		            <div class="col-md-2 col-xs-6">
		                <div class="card label-primary">
		                    <div class="user-box">
		                        ...
		                        <div class="d-flex   no-block align-items-center">
		                            <a href="{{route('reseller.orderHistory', 'on-delivery')}}" class="link ml-auto">{{$on_delivery}}</a>
		                        </div>
		                    </div>
		                </div>
		            </div>
		            <div class="col-md-2 col-xs-6">
		                <div class="card label-danger">
		                    <div class="user-box">
		                        ...
		                        <div class="d-flex  no-block align-items-center">
		                            <a href="{{route('reseller.orderHistory', 'cancel')}}" class="link ml-auto">{{$cancel}}</a>
		                        </div>
		                    </div>
		                </div>
		            </div>
		            <div class="col-md-2 col-xs-6">
		                <div class="card label-success">
		                    <div class="user-box">
		                        ...
		                        <div class="d-flex  no-block align-items-center">
		                            <a href="{{route('reseller.orderHistory', 'delivered')}}" class="link ml-auto">{{$delivered}}</a>
		                        </div>
		                    </div>
		                </div>
		            </div>
...
{{--								<td><a target="_blank" class="dropdown-item" href="{{route('user.orderInvoice', $order->order_id)}}" class="text-inverse" title="View Order Invoice" data-toggle="tooltip"><i class="fa fa-print"></i>Invoice</a></td>--}}
								<td class="text-center">
									<div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li><a class="dropdown-item text-inverse" title="View order" data-toggle="tooltip" href="{{route('reseller.orderDetails', $order->order_id)}}" data-original-title="View"><i class="fa fa-eye"></i> View Details</a></li>

											@if($order->order_status == 'pending' || $order->order_status == 'accepted')
												<li><a title="Cancel Order" onclick="orderCancel('{{ route("reseller.orderCancelForm", [$order->order_id]) }}')" data-toggle="modal" class="dropdown-item" ><i class="fa fa-trash"></i> Cancel order</a></li>
											@endif
                                           
                                            @if($order->order_status == 'delivered')
                                            <li><a title="Return Order" onclick="orderReturn('{{ route("reseller.return.req", [$order->id]) }}')" data-toggle="modal" class="dropdown-item" ><i class="fa fa-trash"></i> Return order</a></li>
                                            @endif
											{{--<li><a title="Cancel Order" onclick="orderCancel('{{ route("reseller.orderCancelForm", [$order->order_id]) }}')" data-toggle="modal" class="dropdown-item" > Address Change Request</a></li>--}}
                                        </ul>
                                    </div> 
								</td>
							</tr>
		...
		<div class="modal-dialog">
			...
			<div class="modal-content">
				...
				<div class="modal-body form-row">
					...
					<div class="card-body">
						<form action="{{route('reseller.orderCancel')}}" onsubmit="return confirm('{{Auth::guard('reseller')->user()->name}} Are you sure cancel this order.?');" method="POST" class="floating-labels">
							{{csrf_field()}}
